    ...
    LAYER: {
        BY: {
            MARKER: {
                 ...
                 * @param thatLayer
                 */
                toThatOutPoint(thisLayer: AVLayer, thatLayer: AVLayer): void {
                    try {
                        const firstMarkerTime: number =
                            GET.MARKER.firstTime_TOF(thisLayer);
                        const gap = GAP.startTimeToTime(
                            thisLayer,
                            firstMarkerTime
                        );
                        const syncPoint: number = thatLayer.outPoint - gap;
                        thisLayer.startTime = syncPoint;
                    } catch (e: any) {
                        throw new Error(
                            `Error in MOVE.LAYER.BY.MARKER.toThatOutPoint.\nLayerNames: ${thisLayer.name}, ${thatLayer.name}.`
                        );
                    }
                },
            },
            INPOINT: {
                ...
                toTime(
                    ...
                    try {
                        const gap = layer.inPoint - layer.startTime;
                        const syncPoint = time - gap + padding;
                        layer.startTime = syncPoint;
                    } catch (e: any) {
                        throw new Error(
                            `Error in MOVE.LAYER.BY.INPOINT.toTime.\nLayerName: ${layer.name}.`
                        );
                    }
                },
            },
